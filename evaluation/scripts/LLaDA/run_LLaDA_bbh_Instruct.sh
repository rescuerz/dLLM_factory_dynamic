...
accelerate launch --config_file accelerate_config.yaml evaluation_script.py -m lm_eval --model LLaDA --tasks bbh --batch_size 2 \
--model_args "pretrained=GSAI-ML/LLaDA-8B-Instruct,prompt_interval_steps=-1,gen_interval_steps=-1,cfg_interval_steps=-1,transfer_ratio=0,is_feature_cache=False,is_cfg_cache=False" \
--gen_kwargs "enable_dynamic_length=True,block_length=256,gen_length=256,steps=256,cfg_scale=0.0 "  \
--num_fewshot 3  \
--output_path ./bbh_log \
--log_samples \
--trust_remote_code \
...
accelerate launch --config_file accelerate_config.yaml evaluation_script.py -m lm_eval --model LLaDA --tasks bbh --batch_size 2 \
--model_args "pretrained=/data/zju-46/zhounan/DLLM/dLLM_factory_dynamic/sft/sft_save/llada-sft/checkpoint-74000,prompt_interval_steps=-1,gen_interval_steps=-1,cfg_interval_steps=-1,transfer_ratio=0,is_feature_cache=False,is_cfg_cache=False" \
--gen_kwargs "enable_dynamic_length=True,block_length=256,gen_length=256,steps=256,cfg_scale=0.0 "  \
--num_fewshot 3  \
--output_path ./bbh_log_lora \
--log_samples \
--trust_remote_code \
--apply_chat_template \
--fewshot_as_multiturn \
