...
import numpy as np
import torch
# import torch.distributed as dist  # 移除未使用的导入
...
class DynamicLengthTrainer(dLLMTrainer):
    ...
    def __init__(self, dynamic_config=None, **kwargs):
        ...
        print(f"🔧 DynamicLengthTrainer initialized with dynamic_length={'enabled' if self.enable_dynamic_length else 'disabled'}")
        print(f"🔧 Dynamic config: {self.dynamic_config}")
    ...
    def _get_special_token_ids(self):
        ...
        token_ids = {}
        # 兼容新版本Transformers：优先使用processing_class，回退到tokenizer
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # steps_in_epoch 表示一个epoch中包含的步数
            steps_in_epoch = (
            ...
            # 将连续的gradient accumulation steps 个 batch 作为一个更新单元
            ...
            for _ in range(total_updates):
                update_step += 1
                # num_batches 表示当前梯度累积组中需要处理的批次数量
                num_batches = args.gradient_accumulation_steps if update_step != (total_updates - 1) else remainder
                # batch_samples 表示当前梯度累积组中的所有批次数据，是一个列表，长度为num_batches
                # num_items_in_batch 表示当前梯度累积组中所有批次数据的样本总数
                batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches)
                for i, inputs in enumerate(batch_samples):
                    # inputs：当前批次的输入数据（包含input_ids, attention_mask等）
                    step += 1
                    # 判断是否需要同步梯度
                    ...
                    with context():
                        # training_step 执行前向传播，损失计算，反向传播，梯度更新
