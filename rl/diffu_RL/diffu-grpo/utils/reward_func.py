# Modified from https://github.com/dllm-reasoning/d1/blob/main/diffu-grpo/reward_func.py
...
def correctness_reward_func(prompts, completions, answer, step=None, run_name=None, **kwargs) -> list[float]:
    ...
    score_list = [2.0 if r == a else 0.0 for r, a in zip(extracted_responses, answer)]
    indices = np.where(np.array(score_list) > 0)[0]
    index = indices[0] if indices.size > 0 else 0
    ...
    print(
        ...
        "-" * 20,
        f"\n{BLUE}{index}-th Response:{RESET}\n{responses[index]}\n",
        "-" * 20,
        f"\n{YELLOW}Extracted:{RESET}\n{extracted_responses[index]}\n",
    )
    print("✅" if is_equiv(extracted_responses[index], answer[index]) else "❌", end=' ')
    print(score_list)
    return score_list
